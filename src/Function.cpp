...
string function_derivate_inner(Term &fkt_arg, const Term &var) {
	...
	if (deriv_arg.is_primitive() || (deriv_arg.fkt()->term().size() == 1L)) {
		deriv_ss << Mult::name << deriv_arg.print();
	} else {
		deriv_ss << Mult::name << term_bracket[0] << deriv_arg.print() << term_bracket[1];
	}
...
string function_evaluate(Term &fkt_arg, const string &fkt_name, const Term &var, const Term &val) {
	...
	eval_ss << fkt_name << function_bracket[0] << fkt_arg.evaluate(var,val).print() << function_bracket[1];
...
string function_simplify(Term &fkt_arg, const string &fkt_name, double (*const fkt)(double)) {
	...
	if (fkt_arg.is_primitive()) {
		if (fkt_arg.is_numeric()) {
			...
			simpl_ss << fkt_name << function_bracket[0] << fkt_arg.print() << function_bracket[1];
		}
	} else {
		simpl_ss << fkt_name << function_bracket[0] << fkt_arg.simplify().print() << function_bracket[1];
	}
...
/** Exp */
Exp::Exp(const string &term_str) : Function() {
...
Term Exp::derivate(const Term &var) {
	...
	deriv_ss << Exp::name << function_bracket[0] << m_args[0]->print() << function_bracket[1];
...
/** Sin */
Sin::Sin(const string &term_str) : Function() {
	m_args.push_back(new Term(term_str));
}
...
Term Sin::derivate(const Term &var) {
	...
	deriv_ss << Cos::name << function_bracket[0] << m_args[0]->print() << function_bracket[1];
	deriv_ss << function_derivate_inner(*m_args[0], var);
...
/** Cos */
Cos::Cos(const string &term_str) : Function() {
	m_args.push_back(new Term(term_str));
}
...
Term Cos::derivate(const Term &var) {
	...
	deriv_ss << Minus::name << Sin::name << function_bracket[0] << m_args[0]->print() << function_bracket[1];
	deriv_ss << function_derivate_inner(*m_args[0], var);
...
/** Tan */
Tan::Tan(const string &term_str) : Function() {
	m_args.push_back(new Term(term_str));
}
...
Term Tan::derivate(const Term &var) {
	...
	deriv_ss << term_bracket[0] << "1" << Plus::name << Tan::name << function_bracket[0] << m_args[0]->print() << function_bracket[1] << Pow::name << "2" << term_bracket[1];
	deriv_ss << function_derivate_inner(*m_args[0], var);
